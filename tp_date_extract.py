...
def label_statistics():
    ...
    e_df = pd.read_csv("../data/ner_tp_email/dtlentities.csv",names=["id","entity","start","end"],sep=",",header = 0)
    c_df = pd.read_csv("../data/try.csv",names=["id","text","agent"],sep=",",header=0)
...
#create_model("../data/try.csv","../data/ner_tp_email/dtlentities.csv")

dictionary = label_statistics()
print(dictionary['dn'])
print(dictionary['dp'])

call_model("Lets meet day after tomorrow")