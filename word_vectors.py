...
def return_word2vec():
    ...
    model = gs.models.Word2Vec.load("../data/word2vec_final/trained_word_vectors.wv")
    #model = gs.models.Word2Vec(X, size=200, min_count=5)
    w2v = dict(zip(model.wv.index2word, model.wv.syn0))
...
class TfIdfEmbedding(object):
    ...
    def convert_query(self,query):
        ...
        return np.array(np.mean([self.word2vec[w] for w in word_tokenize(query) if w in self.word2vec] or [np.zeros(self.dimsize)],axis=0))
